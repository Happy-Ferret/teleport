...
func NewHandler(cfg Config, opts ...HandlerOption) (*RewritingHandler, error) {
	...
	//h.GET("/webapi/sites/:site/namespaces/:namespace/nodes/:node/:login/upload", h.WithClusterAuth(h.uploadFile))

	h.GET("/webapi/sites/:site/namespaces/:namespace/nodes/:node/:login/download", h.WithClusterAuth(h.downloadFile))
